...
var xml2js = __nccwpck_require__(465);
var xml2json = __nccwpck_require__(708);
var js2xml = __nccwpck_require__(379);
...
var helper = __nccwpck_require__(114);
var isArray = (__nccwpck_require__(135).isArray);
...
var sax = __nccwpck_require__(560);
...
const convert = __nccwpck_require__(675);
...
const LOGIN = 'basic'
const action_URL = `https://dev.azure.com/${ORGANIZATION}/{project}/_apis/`
...
async function getWorkItemsWithSprint(sprintId) {
    const url = action_URL.replace('{project}', projectName) + `work/teamsettings/iterations/${sprintId}/workitems?api-version=${API_VERSION}`
...
async function getSprint() {
    const url = action_URL.replace('{project}', projectName) + `work/teamsettings/iterations?$timeframe=current&api-version=${API_VERSION}`
    let headers = new Headers()
    headers.set('Authorization', 'Basic ' + btoa(`${LOGIN}:${TOKEN}`))
    headers.set('Content-Type', 'application/json-patch+json')
    const request = new Request(url, {
...
async function getWorkItemsWithIdList(workItems, fields, filters) {
    const url = action_URL.replace('{project}', projectName) + `wit/workitems?ids=${workItems.join(',')}&api-version=${API_VERSION}${fields ? `&fields=${fields?.join(',')}` : ''}`
    console.log(url)
    let headers = new Headers()
    headers.set('Authorization', 'Basic ' + btoa(`${LOGIN}:${TOKEN}`))
    headers.set('Content-Type', 'application/json-patch+json')
